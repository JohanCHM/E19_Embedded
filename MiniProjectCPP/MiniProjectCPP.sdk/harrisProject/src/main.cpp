...
// double compareFunc(const void * a, const void * b);
...
//	int k;
	...
	// int64_t *test = new int64_t;
	// *test = (long long)585225 * (long long)585225;
	// //	*test = pow((long long)585225,2);
	...
	for (i = 0; i < MSIZE100; i++)
	{
		for (j = 0; j < MSIZE100; j++)
		{
			// if (i==0 && j==0)
			// {
			// 	printf("Problem %d  X %d  = %lld \r\n", *(imgIxx + i * MSIZE100 + j), *(imgIyy + i * MSIZE100 + j), (long long)*(imgIxx + i * MSIZE100 + j) * (long long)*(imgIyy + i * MSIZE100 + j));
			// }
	...
	*max = 0;
	...
	printf("\n\nThresholded Harris Response: %d\r\n", MSIZE100* MSIZE100 - count);
	displayMat(r, 10, 10);

	// corners thresholdedCorners[count];
	...
	// // Maximum supression
	// double *sup = new double[MSIZE100 * MSIZE100];
	// for (i = 0; i < MSIZE100; i++)
	// {
	// 	for (j = 0; j < MSIZE100; j++)
	// 	{
	// 		if (i == 0 || i == MSIZE100 - 1 || j == 0 || j == MSIZE100 - 1)
	// 		{
	// 			*(sup + i * MSIZE100 + j) = 0;
	// 		}
	// 		else
	...
	// printf("\n\nSup:\r\n");
	// displayMat(sup, 20, 10);

	// printf("\n\nSorted:\r\n");

	// qsort(r, MSIZE100 * MSIZE100, sizeof(double), compareFunc);



	// std::array<double, MSIZE100 * MSIZE100> sMat;
	...
	// 	for (j = 0; j < 20; j++)
	// 	{
	// 		sMat[count] = *(r + i * MSIZE100 + j);
	// 		count++;
	// 	}
	// }


	// std::sort(sMat.begin(), sMat.end());
	// for (int a : s) {
    //     std::cout << a << "\n";
    // }


	// for (int i = 1; i < MSIZE100; i++)
	// {
	// 	for (int j = 1; j < MSIZE100; j++)
	// 	{
	// 		// if it si the top border
	// 		if (i == 0)
	// 		{
	// 			if (j == 0) // if it is the left border
	// 			{
	// 				if (*(r + i * MSIZE100 + j) > *(r + i + 1 * MSIZE100 + j) && *(r + i * MSIZE100 + j) > *(r + i * MSIZE100 + j + 1))
	// 				{
	// 					*(sup + i * MSIZE100 + j) = *(r + i * MSIZE100 + j);
	// 				}
	// 				else
	// 				{
	// 					*(sup + i * MSIZE100 + j) = 0;
	// 				}
	// 			}
	// 			else if (j == MSIZE100 - 1) // if it is the right border
	// 			{
	// 				if (*(r + i * MSIZE100 + j) > *(r + i + 1 * MSIZE100 + j) && *(r + i * MSIZE100 + j) > *(r + i * MSIZE100 + j - 1))
	// 				{
	// 					*(sup + i * MSIZE100 + j) = *(r + i * MSIZE100 + j);
	...
	// 			else
	// 			{
	// 				if (*(r + i * MSIZE100 + j) > *(r + i + 1 * MSIZE100 + j) && *(r + i * MSIZE100 + j) > *(r + i * MSIZE100 + j + 1) && *(r + i * MSIZE100 + j) > *(r + i * MSIZE100 + j - 1))
	// 				{
	// 					*(sup + i * MSIZE100 + j) = *(r + i * MSIZE100 + j);
	// 				}
	// 				else
	...
	// 			}
	// 		}
	// 		else if (i == MSIZE100 - 1) // if it is the bottom border
	// 		{
	// 			if (j == 0) // if it is the left border
	// 			{
	// 				if (*(r + i * MSIZE100 + j) > *(r + i - 1 * MSIZE100 + j) && *(r + i * MSIZE100 + j) > *(r + i * MSIZE100 + j + 1))
	...
	// 			if (j == 0) // if it is the left border
	// 			{
	// 				if (*(r + i * MSIZE100 + j) > *(r + i - 1 * MSIZE100 + j) && *(r + i * MSIZE100 + j) > *(r + i + 1 * MSIZE100 + j) && *(r + i * MSIZE100 + j) > *(r + i * MSIZE100 + j + 1))
	// 				{
	// 					*(sup + i * MSIZE100 + j) = *(r + i * MSIZE100 + j);
	// 				}
	// 				else
	...
	// 			else
	// 			{
	// 				if (*(r + i * MSIZE100 + j) > *(r + i - 1 * MSIZE100 + j) && *(r + i * MSIZE100 + j) > *(r + i + 1 * MSIZE100 + j) && *(r + i * MSIZE100 + j) > *(r + i * MSIZE100 + j + 1) && *(r + i * MSIZE100 + j) > *(r + i * MSIZE100 + j - 1))
	// 				{
	// 					*(sup + i * MSIZE100 + j) = *(r + i * MSIZE100 + j);
	// 				}
	// 				else
